...
larger than 0, which is considered to be an absolute range length. In
...
the lower section containing the range dat in genomic coordinates, and
the upper one containing the additional data for each of the displayed
ranges in verticaly tiled panels of equal size. The conenction between
...
Finaly, it is as simple as calling the \Rclass{DetailsAnnotationTrack}
constructor (which mostly resembles the familiar
\Rclass{AnnotationTrack} constructor), passing along the wrapper function
and calling \Rfunction{plotTracks}.

<<DetailsAnnotationTrack4, results=hide, width=7.5, height=1, fig=TRUE>>=
deTrack <- DetailsAnnotationTrack(range=probes, genome="hg19", chromosome=7, id=rownames(methylation),
...
it along using the \Rfunargs{detailFunArgs} parameter, or making it
...
<<DetailsAnnotationTrack5, results=hide, width=7.5, height=1, fig=TRUE>>=
plotTracks(deTrack, details.size=0.25, detailsConnector.pch=NA, detailsConnector.col="darkred", 
           detailsBorder.fill="#FFE3BF", detailsBorder.col="darkred", shape="box", detailsConnector.lty="dotted")
@ 

<<DeatailsAnnotationTrackClassTable, echo=FALSE, results=tex>>=
addParTable("BiomartGeneRegionTrack")
@ 
