...
<<typeGroupedPlots, fig=TRUE, width=7.5, height=5.6, echo=FALSE, results=hide>>=
...
\code{elementMetadata} column. \code{bedGraph} files are not indexed,
...
object. In other words, numeric \code{elementMetadata} columns will be
...
position only and to encode this in a single \code{elementMetadata}
column.
...
\Rclass{elementMetadata} columns of the \Rclass{GRanges} object that is
...
scalars, the \Rclass{elementMetadata} columns of the \Rclass{GRanges}
...
Please note that if no \Rclass{elementMetadata} can be mapped to the
...
and  read  pairs are  connect  by  a bright  gray  line.  Gaps in  the
alignments are show by the connecting dark gray lines. On devices that
support transparancy we  can also see that some of  the read pairs are
actually overlapping. Like  before, we can not display  the full depth
of the pile-up,  and the clipping that takes place hides  a lot of the
reads from  the view. Another way of  getting to see at  least some of
them is by reversing the stacking  order. We can also play around with
some of the other display parameters that control the look and feel of
the reads.
