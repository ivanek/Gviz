...
plotTracks(list(itrack, gtrack, atrack, grtrack, strack), 
           from = 159024485, to = 159231338, cex = 0.8)
```
...
```{r filedt2}
library(rtracklayer)
dTrack3 <-  DataTrack(range = bgFile, genome = "hg38", type = "l", 
                      chromosome = "chr19", name = "bedGraph",
...
dTrack4 <- DataTrack(range = bamFile, genome = "hg38", type = "l", 
...
DataTrack(range = bamFile, genome = "hg38", type = "l",
...
aTrack2 <- AnnotationTrack(range = bamFile, genome = "hg38", 
...
aTrack3 <- AnnotationTrack(range = bamFile, genome = "hg38", 
                           name = "Reads", chromosome = "chr1", 
                           group = "id")
aTrack3
...
plotTracks(grtrack, exonAnnotation = "exon", extend.left = -.98, 
           fontcolor.exon = 1)
```
...
plotTracks(eTrack)
options(ucscChromosomeNames=TRUE) # TODO FIX 
```
...
be selected in that object. In the example code below, the older version of human genome assembly (hg38) and matching Biomart version are used.
...
biomTrack <- BiomartGeneRegionTrack(chromosome = "chr7", 
                                    start = 20.75e6, end = 21e6,
...
biomTrack <- BiomartGeneRegionTrack(name = "ENSEMBL", 
                                    symbol = "ABCB5", 
                                    biomart = bm)
...
biomTrack <- BiomartGeneRegionTrack(genome="hg38", chromosome="chr7", start=20e6, end=21e6,name="ENSEMBL",
...
deTrack <- AnnotationTrack(range = probes, genome = "hg38", 
...
`SequenceTrack` from the human hg38 UCSC genome.
...
an extract of the human hg38 genome. The data originate from an RNASeq
...
alTrack <- AlignmentsTrack(
  system.file(package = "Gviz", "extdata", "gapped.bam"),
  isPaired = TRUE)
bmt <- BiomartGeneRegionTrack(chromosome = "chr12",
                              start = afrom, end = ato,
                              filter = list(with_refseq_mrna = TRUE),
                              stacking = "dense", biomart = bm)
```
...
is hg38 and the alignments have been performed using `Bowtie2`.
