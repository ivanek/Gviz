...
output:
  ...
  BiocStyle::pdf_document: 
    default
vignette: >
    %\VignetteIndexEntry{Gviz User Guide}
...
__Compiled date__: `r BiocStyle::doc_date()`

__Last edited__: 10 December 2018

<!-- 
__Last edited__: `r format(as.Date(system("git log -1 --format='%ai' vignettes/Gviz.Rnw", intern = TRUE)), "%d %B %Y")`
-->

<!--
knit: (function(inputFile, encoding) {rmarkdown::render(input=inputFile, encoding = encoding,  output_format='BiocStyle::html_document',output_file=paste0(dirname(inputFile),'/',basename(tools::file_path_sans_ext(inputFile)),'.html'));rmarkdown::render(input=inputFile, encoding = encoding,  output_format='BiocStyle::pdf_document',output_file=paste0(dirname(inputFile),'/',basename(tools::file_path_sans_ext(inputFile)),'.pdf'));})
-->
...
addParTable <- function(class, skip=c("showTitle", "size", "background.title"), add=NULL, out_type="html") {
  ...
  if (out_type == "html") {
    ...
    out <- kable(Parameters, "latex", booktabs=TRUE, longtable=TRUE) %>% kable_styling(full_width = FALSE, latex_options = "hold_position")
  }
  print(out)
  return(invisible())
}
...
numeric data in a multitude of different ways. The `r Biocpkg("Gviz")` package aims
...
on the `r Biocpkg("GenomeGraphs")` package by Steffen Durinck and James
...
[UCSC](https://genome.ucsc.edu/) or [ENSEMBL](https://www.ensembl.org/).
...
coordinate tickmarks. Using the `GenomeAxisTrack`
...
```{r GenomeAxisTrackClass1, fig.width=7.5, fig.height=0.5}
...
```{r GenomeAxisTrackClass2, fig.width=7.5, fig.height=0.5}
...
```{r GenomeAxisTrackClass2a, fig.width=7.5, fig.height=0.5}
...
```{r GenomeAxisTrackClass3, fig.width=7.5, fig.height=0.5}
...
```{r GenomeAxisTrackClass4, fig.width=7.5, fig.height=0.5}
...
```{r GenomeAxisTrackClass5, fig.width=7.5, fig.height=0.5}
...
```{r GenomeAxisTrackClass6, fig.width=7.5, fig.height=0.5}
...
addParTable("GenomeAxisTrack", out_type = out_type)
```
...
addParTable("IdeogramTrack", out_type = out_type)
```

## DataTrack

Probably the most powerfull of all the track classes in the `r Biocpkg("Gviz")`
...
`DataTrack` is a rather unispiring dot plot. The track comes
...
page for details. ^[PLese note, that in the example, a different data 
...
than zero, and the addtional display parameter `windowSize`
...
addParTable("DataTrack", out_type = out_type)
```
...
accomodate all of them. This involves some optimization, and the `r Biocpkg("Gviz")`
...
`fill` display paramter. To define colors for individual
...
the `r Biocpkg("Gviz")` package, however it implies that unless neighbouring items
...
addParTable("AnnotationTrack", out_type = out_type)
```
...
`start` and `end` argument take on a slightly differnt
...
```{r generegtrack4f, results='hide', fig.width=7.5, fig.height=0.35}
feature(txTr)
plotTracks(txTr)
```
...
addParTable("GeneRegionTrack", out_type = out_type)
```
...
```{r BiomartGeneRegionTrackCustom, fig.width=7.5, fig.height=1.25}
library(biomaRt)
...
addParTable("BiomartGeneRegionTrack", add=addInfo, , out_type = out_type)
```
...
and environement or even from a `GRanges` object which will
...
Finaly, it is as simple as calling the `AnnotationTrack`
...
In addtion to the display parameters for the `AnnotationTrack`
...
addParTable("DetailsAnnotationTrack", out_type = out_type)
```
...
paramter to `TRUE` to always force the drawing of boxes.  Of
...
addParTable("SequenceTrack", out_type = out_type)
```
...
agument to the `type` display parameter.
...
addParTable("AlignmentsTrack", out_type = out_type)
```
...
interesting behaviour. Because the individual tracks in the `r Biocpkg("Gviz")`
...
class by typing in `?HighlightTrack` on the \RR command line.

```{r HighlightTrackClassTable, echo=FALSE, results='asis'}
addParTable("HighlightTrack", out_type = out_type)
```
