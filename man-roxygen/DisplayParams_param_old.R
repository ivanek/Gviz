...
#' @param cex.title=NULL Numeric scalar. The expansion factor for the title
...
#' @param fontface.title=2 Integer or character scalar. The font face for the
#' title panels.
#' @param fontfamily="sans" Integer or character scalar. The font family for all
#' text, unless a more specific definition exists.
#' @param fontfamily.title="sans" Integer or character scalar. The font family
#' for the title panels.
#' @param fontsize=12 Numeric scalar. The font size for all text, unless a more
#'  specific definition exists.
#' @param frame=FALSE Boolean. Draw a frame around the track when plotting.
#' @param grid=FALSE Boolean, switching on/off the plotting of a grid.
#' @param h=-1 Integer scalar. Parameter controlling the number of horizontal
#' grid lines, see panel.grid for details.
#' @param lineheight=1 Numeric scalar. The font line height for all text, unless
#'  a more specific definition exists.
#' @param lty="solid" Numeric scalar. Default line type setting for all plotting
#' elements, unless there is a more specific control defined elsewhere.
#' @param lty.grid="solid" Integer or character scalar. Default line type for
#'  grid lines, both when `type=="g"` in `DataTrack`s and when display parameter
#'  `grid==TRUE`.
#' @param lwd=1 Numeric scalar. Default line width setting for all plotting
#'  elements, unless there is a more specific control defined elsewhere.
#' @param lwd.border.title=1 Integer scalar. The border width for the title
#' panels.
#' @param lwd.grid=1 Numeric scalar. Default line width for grid lines, both
#' when `type=="g"` in `DataTrack`s and when display parameter `grid==TRUE`.
#' @param lwd.title=1 Integer scalar. The border width for the title panels
#' @param min.distance=1 Numeric scalar. The minimum pixel distance before
#' collapsing range items, only if `collapse==TRUE`. See collapsing for details.
#' @param min.height=3 Numeric scalar. The minimum range height in pixels to
#' display. All ranges are expanded to this size in order to avoid rendering
#'  issues. See collapsing for details.
#' @param min.width=1 Numeric scalar. The minimum range width in pixels to
#' display. All ranges are expanded to this size in order to avoid rendering
#' issues. See collapsing for details.
#' @param reverseStrand=FALSE Logical scalar. Set up the plotting coordinates
#'  in 3' -> 5' direction if `TRUE.` This will effectively mirror the plot
#'   on the vertical axis.
#' @param rotation=0 The rotation angle for all text unless a more specific
#' definition exists.
#' @param rotation.title=90 (Aliases rotation.title) The rotation angle for
...
#' @param showAxis=TRUE Boolean controlling whether to plot a y axis (only
#' applies to track types where axes are implemented).
#' @param showTitle=TRUE Boolean controlling whether to plot a title panel.
