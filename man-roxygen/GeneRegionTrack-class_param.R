...
#' @param object Object inheriting from `GeneRegionTrack-class`.
...
#' `rstarts` and `rends` for the currently active chromosome. When
#' building a `GeneRegionTrack` from a `TxDb` object, these arguments
#' can be used to subset the desired annotation data by genomic coordinates.
#' Please note this in that case the `chromosome` parameter must also be
#' set.
...
#' exons is handled via the `gene` and `transcript` factors.
...
#' model items. Both `rstarts` and `rends` have to be of equal
#' length.
#' @param rwidths An integer vector of widths for the actual gene model items.
#' This can be used instead of either `rstarts` or `rends` to specify
...
#' `GeneRegionTrack` is considered to be coding or non-coding. The details
#' section as well as the section about the `thinBoxFeature` display
...
#' `-` for the Crick strand or `*` for either one of the two. Please
...
#' to the device.  See [`grouping`] for details. Will be used as
#' labels when `showId=TRUE`, and `geneSymbol=FALSE`.
...
#' `GeneRegionTrack` from a `TxDb` object, the value of this
...
#' `c(hide, dense, squish, pack,full)`. Currently, only hide (don't show
...
#' functions: `gff`, `gff1`, `gff2`, `gff3`, `gtf`.
...
#' constructor to return a `ReferenceGeneRegionTrack` object which will
