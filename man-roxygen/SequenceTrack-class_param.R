...
#' @param minBase Numeric scalar, the start coordinate of the sequence.
#' @param maxBase Numeric scalar, the end coordinate of the sequence.
...
#' @param object Object inheriting from [`SequenceTrack`][SequenceTrack-class].
...
#' chromosome identifier if `options(ucscChromosomeNames=TRUE)`. Please note
#' that in this case only syntactic checking takes place, i.e., the argument
#' value needs to be an integer, numeric character or a character of the form
#' `chrx`, where `x` may be any possible string. The user has to make sure that
#' sequences for the respective chromosomes are indeed part of the object. If
#' not provided here, the constructor will set it to the first available
#' sequence. Please note that by definition all objects in the `Gviz` package
#' can only have a single active chromosome at a time (although internally the
...
#' [`SequenceBSgenomeTrack`][SequenceBSgenomeTrack-class] object, the genome
#' information is extracted from the input
#' [`BSgenome`][BSgenome::BSgenome-class] package. For a
#' [`DNAStringSet`][Biostrings::DNAStringSet-class] it has to be provided, or
...
#' proper [`DNAStringSet`][Biostrings::DNAStringSet-class] object with the
...
#' [`ReferenceSequenceTrack`][ReferenceSequenceTrack-class] class, which
...
#' [`GRanges`][GenomicRanges::GRanges-class] object containing the dimensions of
#' the plotted genomic range. As before, the function has to return an
#' appropriate [`DNAStringSet`][Biostrings::DNAStringSet-class] object.
...
#' [`ReferenceSequenceTrack`][ReferenceSequenceTrack-class] object which will
