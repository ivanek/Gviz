...
#' the `Annotation` track as well as the optional additional metadata columns
#' `feature`, `group` and `id` (see description of the individual function
#' parameters below for details). Calling the constructor on a
#' [`GRanges`][GenomicRanges::GRanges-class] object without further arguments,
#' e.g. `AnnotationTrack(range=obj)`, is equivalent to calling the coerce method
...
#' part of the list structure is preserved in the
#' [`AnnotationTrack`][AnnotationTrack-class]. I.e., all the elements within one
#' list item receive the same group id. For consistency, there is also a
#' coercion method from [`GRangesList`][GenomicRanges::GRangesList-class]
#' objects, `as(obj, "AnnotationTrack")`.
...
#' and strand information as well as all additional metadata has to be provided
#' in the separate `chromosome`, `strand`, `feature`, `group` or `id` arguments,
#' because it cannot be directly encoded in an
#' [`IRanges`][IRanges::IRanges-class] object. Note that none of those inputs
#' are mandatory, and if not provided explicitly the more or less reasonable
#' default values `chromosome=NA` and `strand="*"` are used.
...
#' information for each range. If missing, it will be drawn from the separate
#' `chromosome` or `strand` arguments. In addition, the `feature`, `group` and
#' `id` data can be provided as additional columns. The above comments about
...
#' `feature` is set, this will be used as the track item's fill color. See
#' [`grouping`] for details. Needs to be of equal length as the provided genomic
...
#' device, all items in the same group will be connected. See [`grouping`] for
#' details. Needs to be of equal length as the provided genomic coordinates, or
#' of length 1.
#' @param id Character vector of track item identifiers. When plotting to the
#' device, its value will be used as the identifier tag if the display
#' parameter `showFeatureId=TRUE`. Needs to be of equal length as the provided
#' genomic ranges, or of length 1.
...
#' for the Crick strand or `*` for either one of the two. Needs to be of equal
#' length as the provided genomic coordinates, or of length 1. Please note that
#' grouped items need to be on the same strand, and erroneous entries will
#' result in an error.
...
#' imported with the default functions: `gff`, `gff1`, `gff2`, `gff3`, `bed`,
#' `bam`.
...
#' [`ReferenceAnnotationTrack`][ReferenceAnnotationTrack-class] object which
