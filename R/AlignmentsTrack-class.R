...
setClass(
    ...
    contains = c("AlignmentsTrack", "ReferenceTrack")
)
  ...
  ...
    ...
    if (!is.character(range)) {
        ...
        id <- .covDefault(
            id,
            covars[["id"]],
            paste("read", seq_len(n), sep = "_")
        )
        cigar <- .covDefault(
            cigar,
            covars[["cigar"]],
            paste(
                if (is(range, "GRangesOrIRanges")) width(range) else width,
                "M",
                sep = ""
            )
        )
        ...
        status <- .covDefault(
            ...
            covars[["status"]],
            ifelse(
                groupid %in% groupid[duplicated(groupid)],
                "mated",
                "unmated"
            )
        )
    ...
    ## This is going to be a list if we have to stream data from a file,
    ## otherwise we can compute some additional values
    if (is.list(range)) {
        isStream <- TRUE
        slist <- range
        range <- GRanges()
        ...
        if (is.null(seqs)) {
            seqs <- DNAStringSet(vapply(
                width(range),
                function(x) paste(rep("N", x), collapse = ""),
                character(1)
            ))
        }
        addArgs <- list(...)
...
#' a `data.frame`.
#' @export
setMethod("values", "AlignmentsTrack", function(x) {
    .dpOrDefault(x, ".__coverage")
})
...
setMethod("setStacks", "AlignmentsTrack", function(GdObject, ...) {
    if (length(GdObject)) {
        bins <- if (!.needsStacking(GdObject)) {
            ...
            disjointBins(GdObject@stackRanges)
        }
        GdObject@stacks <- bins
        ranges(GdObject)$stack <- bins[match(
            ranges(GdObject)$groupid,
            names(GdObject@stackRanges)
        )]
    }
    return(GdObject)
})
...
## Collapse  -----------------------------------------------------------------
## Subset --------------------------------------------------------------------
## AlignmentTracks can be subset by using the information in the stackRanges
## slot, but for the actual reads we need to make sure that
## we keep all the bits that belong to a given group.
...
setMethod(
    ...
    function(x,
        ...
        if (length(x)) {
            ## Nothing to do if everything is within the range, otherwise
            ## we subset the stackRanges and keep all group items
            ...
            sr <- subsetByOverlaps(
                x@stackRanges,
                GRanges(
                    seqnames = chromosome(x)[1],
                    ranges = IRanges(start = ranges["from"], end = ranges["to"])
                )
            )
...
## ReferenceAlignmentsTracks need to stream the data from file
...
setMethod(
    "subset",
    signature(x = "ReferenceAlignmentsTrack"),
    function(x, from, to, chromosome, ...) {
        ...
        if (!isIn) {
            ...
            range <- .computeAlignments(
                ...
                drop.D.ranges = .dpOrDefault(x, "showIndels", FALSE)
            )
...
setMethod(
    ...
    function(GdObject, from, to) {
        if (.dpOrDefault(GdObject, "grid", FALSE)) {
            ...
            yscale <- c(
                if (is.null(.dpOrDefault(GdObject, "transformation"))) {
                    0
                } else {
                    min(ylim)
                },
                max(ylim) + diff(range(ylim)) * 0.05
            )
            covHeight <- .dpOrDefault(GdObject, ".__coverageHeight", 0)
            pushViewport(viewport(
                y = 1 - covHeight["npc"],
                height = covHeight["npc"],
                just = c(0.5, 0),
                yscale = yscale,
                clip = TRUE
            ))
...
setMethod(
    ...
    function(GdObject, ...) {
        type <- match.arg(
            .dpOrDefault(GdObject, "type", .ALIGNMENT_TYPES),
            .ALIGNMENT_TYPES,
            several.ok = TRUE
        )
        if ("coverage" %in% type) {
            ...
            pushViewport(viewport(
                y = 1 - (covHeight["npc"] + covSpace),
                height = covHeight["npc"],
                just = c(0.5, 0)
            ))
            if (is.null(acex)) {
                vSpaceNeeded <- max(as.numeric(convertWidth(
                    stringHeight(at),
                    "inches"
                ))) *
                    length(at) *
                    1.5
                hSpaceNeeded <- max(as.numeric(convertWidth(
                    stringWidth(at),
                    "inches"
                )))
                vSpaceAvail <- abs(diff(range(at))) /
                    abs(diff(yscale)) *
                    vpLocation()$isize["height"]
                acex <- max(
                    0.6,
                    min(vSpaceAvail / vSpaceNeeded, hSpaceAvail / hSpaceNeeded)
                )
            }
        ...
        if ("sashimi" %in% type) {
            ...
            yscale <- if (length(sash$y)) {
                c(-(max(sash$y) + diff(range(sash$y)) * 0.05), 0)
            } else {
                c(-1, 0)
            }
            ylim <- if (length(sash$y)) {
                c(-max(sash$y), yscale[1] + max(sash$y))
            } else {
                c(-1, 0)
            }
            hSpaceAvail <- vpLocation()$isize["width"] / 6
            ...
            acex <- .dpOrDefault(GdObject, "cex.axis")
            acol <- .dpOrDefault(GdObject, "col.axis", "white")
            labs <- if (length(sash$score)) {
                pretty(c(1, sash$score))
            } else {
                pretty(c(1, .dpOrDefault(GdObject, ".__sashimiScore", 10)))
            }
            at <- seq(ylim[1], ylim[2], length.out = length(labs))
            sashHeight <- .dpOrDefault(
                GdObject,
                ".__sashimiHeight",
                c(npc = 0, points = 0)
            )
...
setMethod(
    ...
    function(GdObject, minBase, maxBase, prepare = FALSE, subset = TRUE, ...) {
        ...
        ## StackedTrack drawGD method), and compute the coverage vector
        ## swhich will be needed for the axis
        if (prepare) {
            GdObject <- callNextMethod(GdObject, ...)
            ## The mismatched bases need to be extracted
            ## from the read sequences and the reference sequence
            if (
                .dpOrDefault(GdObject, "showMismatches", TRUE) &&
                    !is.null(GdObject@referenceSequence)
            ) {
            ...
            if ("coverage" %in% type) {
                ...
                if ("pileup" %in% type) {
                    ...
                    if (covHeight > 0 && covHeight < 1) {
                        covHeight <- as.numeric(convertHeight(
                            unit(covHeight, "npc"),
                            "points"
                        ))
                    }
                    covHeight <- max(
                        .dpOrDefault(GdObject, "minCoverageHeight", 50),
                        covHeight
                    )
                    covHeight <- c(
                        points = covHeight,
                        npc = as.numeric(convertHeight(
                            unit(covHeight, "points"),
                            "npc"
                        ))
                    )
            ...
            if ("sashimi" %in% type) {
                ...
                if ("pileup" %in% type) {
                    ...
                    if (sashHeight > 0 && sashHeight < 1) {
                        sashHeight <- as.numeric(convertHeight(
                            unit(sashHeight, "npc"),
                            "points"
                        ))
                    }
                    sashHeight <- max(
                        .dpOrDefault(GdObject, "minSashimiHeight", 50),
                        sashHeight
                    )
                    sashHeight <- c(
                        points = sashHeight,
                        npc = as.numeric(convertHeight(
                            unit(sashHeight, "points"),
                            "npc"
                        ))
                    )
                ...
                sashFilterTolerance <- .dpOrDefault(
                    GdObject,
                    "sashimiFilterTolerance",
                    0L
                )
                sashNumbers <- .dpOrDefault(GdObject, "sashimiNumbers", FALSE)
                sash <- .dpOrDefault(GdObject, "sashimiJunctions", NULL)
                if (is.null(sash)) {
                    sash <- .create.summarizedJunctions.for.sashimi.junctions(ranges(
                        GdObject
                    ))
                } else {
                    if (!is(sash, "GRanges")) {
                        stop(
                            "\"sashimiJunctions\" object must be of \"GRanges\" class!"
                        )
                    }
                    sashMcolName <- if (sashStrand == "+") {
                        "plus_score"
                    } else if (sashStrand == "-") {
                        "minus_score"
                    } else {
                    ...
                    if (sum(colnames(mcols(sash)) == sashMcolName) != 1) {
                        stop(sprintf(
                            "\"mcols\" of \"sashimiJunctions\" object must contain column named \"%s\",\n which matches the specified (%s) \"sashimiStrand\"!",
                            sashMcolName,
                            sashStrand
                        ))
                    }
                }
                sashTransform <- .dpOrDefault(
                    GdObject,
                    c("sashimiTransformation", "transformation")
                )
            ...
            if ("pileup" %in% type) {
                ...
                mih <- min(
                    curVp$size["height"],
                    .dpOrDefault(GdObject, c("min.height", "max.height"), 3)
                )
                mah <- min(
                    curVp$size["height"],
                    max(
                        mih,
                        .dpOrDefault(GdObject, c("max.height", "min.height"), 8)
                    )
                )
                ...
                yrange <- if (revs) {
                    c(yrange[1], yrange[2] + add)
                } else {
                    c(yrange[1] - add, yrange[2])
                }
        ...
        if ("coverage" %in% type) {
            cov <- .dpOrDefault(
                GdObject,
                ".__coverage",
                Rle(lengths = maxBase, values = as.integer(0))
            )
            yscale <- c(
                if (is.null(.dpOrDefault(GdObject, "transformation"))) {
                    0
            ...
            if (res > 2) {
                ...
                y <- tapply(
                    as.integer(cov[mminBase:maxBase]),
                    cut(mminBase:maxBase, breaks = brks),
                    mean
                )
            } else {
                x <- mminBase:maxBase
            ...
            grid.polygon(
                c(minBase - max(1, res), 0, x, maxBase + max(1, res)),
                c(0, 0, y, 0),
                default.units = "native",
                gp = gp
            )
            ...
            if (!is.null(mm)) {
                ...
                grid.rect(
                    x = x,
                    y = y,
                    height = unlist(vpos, use.names = FALSE),
                    width = 1,
                    default.units = "native",
                    just = c(0, 1),
            ...
            vp <- viewport(
                height = twoPx,
                y = 1 - (covHeight["npc"] + covSpace + twoPx),
                just = c(0.5, 0)
            )
            pushViewport(vp)
            grid.rect(
                gp = gpar(
                    fill = .dpOrDefault(GdObject, "background.title"),
                    col = "transparent",
        ...
        if ("sashimi" %in% type) {
            ...
            yscale <- if (length(sash$y)) {
                c(-(max(sash$y) + diff(range(sash$y)) * 0.15), 0)
            } else {
                c(-1, 0)
            } # changed from 0.05 to 0.15 to make sure that numbers fit in the viewport
            vp <- viewport(
                height = sashHeight["npc"],
                y = 1 -
                    (covHeight["npc"] +
            ...
            if (length(sash$x)) {
                grid.xspline(
                    ...
                    open = TRUE,
                    default.units = "native",
                    gp = gpar(col = gp$col, lwd = sash$scaled)
                )
                ## print the number of reads together with the connecting lines
                ## (currently no scaling/resolution)
                if (sashNumbers) {
                    grid.rect(
                        sash$x[c(FALSE, TRUE, FALSE)],
                        -sash$y[c(FALSE, TRUE, FALSE)],
                        width = convertUnit(
                            stringWidth(sash$score) * 1.5,
                            "inches"
                        ),
                        height = convertUnit(
                            stringHeight(sash$score) * 1.5,
                            "inches"
                        ),
                        default.units = "native",
                        gp = gpar(col = gp$col, fill = gp$fill)
                    )
                    grid.text(
                        label = sash$score,
                        sash$x[c(FALSE, TRUE, FALSE)],
        ...
        if ("pileup" %in% type) {
            cex <- max(
                0.3,
                .dpOrDefault(GdObject, c("cex.mismatch", "cex"), 0.7)
            )
            pushViewport(viewport(
                height = 1 -
                    (covHeight["npc"] +
            ...
            if (boxOnly) {
                ...
                uid2strand <- setNames(
                    as.character(readInfo$readStrand),
                    as.character(readInfo$uid)
                )
                arrowMap <- unlist(setNames(
                    lapply(
                        split(as.character(readInfo$uid), readInfo$entityId),
                        function(x) {
                            str <- uid2strand[x][1]
                            setNames(
                                str,
                                if (str == "+") tail(x, 1) else head(x, 1)
                            )
                        }
                    ),
                    NULL
                ...
                x <- c(
                    start(readInfo)[sel],
                    rep(end(readInfo)[sel] + 1, 2),
                    start(readInfo)[sel]
                )
                y <- c(
                    rep(readInfo$stack[sel] + sh, 2),
                    rep(readInfo$stack[sel] - sh, 2)
                )
            ...
            gps <- data.frame(
                ...
                fill = rep(
                    .dpOrDefault(
                        GdObject,
                        c("fill.reads", "fill"),
                        .DEFAULT_BRIGHT_SHADED_COL
                    ),
                    nn
                ),
                lwd = rep(.dpOrDefault(GdObject, c("lwd.reads", "lwd"), 1), nn),
                lty = rep(.dpOrDefault(GdObject, c("lty.reads", "lty"), 1), nn),
                alpha = rep(.alpha(GdObject, "reads"), nn),
                stringsAsFactors = FALSE
            )
            ## Finally we draw reads (we need to draw in two steps
            ## because of the different alpha levels, reads vs. mismatches)
            grid.polygon(
                x = x,
                y = y,
                id = id,
            ...
            if (anyDuplicated(readInfo$entityId) != 0) {
                ...
                mateGaps <- gaps(GRanges(
                    ranges = ranges(readInfo),
                    seqnames = readInfo$entityId
                ))
                rmap <- mateRanges[as.character(seqnames(mateGaps))]
                mateGaps <- mateGaps[
                    start(rmap) <= start(mateGaps) & end(rmap) >= end(mateGaps)
                ]
                gy <- readInfo$stack[match(
                    as.character(seqnames(mateGaps)),
                    readInfo$entityId
                )]
            ...
            if (
                any(readInfo$status != "unmated") &&
                    anyDuplicated(readInfo$groupid) != 0
            ) {
                pairGaps <- gaps(GRanges(
                    ranges = ranges(mateRanges),
                    seqnames = readInfo$groupid[match(
                        names(mateRanges),
                        readInfo$entityId
                    )]
                ))
                ...
                pairGaps <- pairGaps[
                    start(rmap) <= start(pairGaps) & end(rmap) >= end(pairGaps)
                ]
                gy <- readInfo$stack[match(
                    as.character(seqnames(pairGaps)),
                    readInfo$groupid
                )]
                if (length(pairGaps)) {
                ...
                lineCoords <- rbind(
                    lineCoords,
                    pairsCoords[!duplicated(pairsCoords), ]
                )
            }
            ...
            if (showIndels) {
                cigarTmp <- DataFrame(
                    cigar = readInfo$cigar,
                    start = start(readInfo),
                    entityId = readInfo$entityId,
                    groupId = readInfo$groupid
                )
                cigarTmp <- cigarTmp[order(cigarTmp$entityId, cigarTmp$start), ]
                cigarTmp <- cigarTmp[!duplicated(cigarTmp$entityId), ]
                delGaps <- unlist(cigars_as_ranges_along_ref(
                    cigarTmp$cigar,
                    lmmpos = cigarTmp$start,
                    ops = "D",
                    f = as.factor(cigarTmp$entityId)
                ))
                ...
                if (length(delGaps)) {
                    delCoords <- data.frame(
                        ...
                        alpha = .alpha(GdObject, "deletions"),
                        stringsAsFactors = FALSE
                    )
                    lineCoords <- rbind(delCoords, lineCoords)
                    lineCoords <- lineCoords[
                        !duplicated(lineCoords[, c("x1", "y1", "x2", "y2")]),
                    ]
                }
                insGaps <- unlist(cigars_as_ranges_along_ref(
                    cigarTmp$cigar,
                    lmmpos = cigarTmp$start,
                    ops = "I",
                    f = as.factor(cigarTmp$entityId)
                ...
                if (length(insGaps)) {
                    ...
                    insCoords <- data.frame(
                        ...
                        alpha = .alpha(GdObject, "insertions"),
                        stringsAsFactors = FALSE
                    )
                }
            }
            ...
            if (!is.null(mm)) {
                ...
                lwidth <- max(as.numeric(convertUnit(
                    ...
                    0.9337632
                lheight <- max(as.numeric(convertUnit(
                    stringHeight(DNA_ALPHABET),
                    "inches"
                )))
                perLetterW <- vpl$isize["width"] / (maxBase - minBase + 1)
                perLetterH <- vpl$isize["height"] /
                ...
                if (nrow(mm)) {
                    x <- c(
                        mm$position + rep(c(0, mwy, mwy, 0), each = nrow(mm))
                    )
                    y <- c(rep(mm$stack - sh, 2), rep(mm$stack + sh, 2))
                    id <- c(rep(
                        seq(max(id, na.rm = TRUE) + 1, len = nrow(mm)),
                        4
                    ))
                    gps <- data.frame(
                        col = rep(
                            if (
                                !(lwidth < perLetterW && lheight < perLetterH)
                            ) {
                                "transparent"
                            } else {
                                .dpOrDefault(
                        ...
                        lwd = rep(
                            .dpOrDefault(GdObject, "lwd.mismatch", 1),
                            nrow(mm)
                        ),
                        lty = rep(
                            .dpOrDefault(GdObject, "lty.mismatch", 1),
                            nrow(mm)
                        ),
                        alpha = rep(
                            .dpOrDefault(GdObject, "alpha.mismatch", 1),
                            nrow(mm)
                        ),
                        stringsAsFactors = FALSE
                    )
                    ## Finally we draw mm (we need to draw in two steps
                    ## because of the different alpha levels,
                    ## reads vs. mismatches)
                    grid.polygon(
                        x = x,
                        y = y,
            ...
            if (!is.null(mmLetters)) {
                grid.text(
                    x = mmLetters$x,
                    y = mmLetters$y,
                    label = mmLetters$label,
                    gp = gpar(col = mmLetters$col),
                    default.units = "native"
                )
            }
            popViewport(2)
...
setMethod(
    "show",
    signature(object = "ReferenceAlignmentsTrack"),
    function(object) {
