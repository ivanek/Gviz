...
setMethod(
    "drawAxis",
    signature(GdObject = "NumericTrack"),
    function(GdObject, from, to, ...) {
        ...
        if (
            ...
            sn <- if (is.null(groups)) {
                ...
                rev(unlist(split(rownames(values(GdObject)), factor(groups))))
            }
        ...
        if (
            any(type %in% c("gradient", "heatmap")) &&
                .dpOrDefault(GdObject, "showColorBar", TRUE)
        ) {
            ...
            vpAxisCont <- viewport(
                x = unit(1, "npc") - unit(2 - shift, "points"),
                width = unit(1, "npc") - unit(2 - shift, "points"),
                just = 1
            )
            ...
            for (i in seq_len(nlevs)) {
                ...
                vpTitleAxis <- viewport(
                    x = unit(1, "npc") - unit(4 * (i - 1), "points"),
                    width = unit(4 + pshift, "points"),
                    yscale = yscale,
                    just = 1
                )
                ...
                if (all(type %in% c("gradient", "heatmap"))) {
                    ...
                    grid.rect(
                        y = unit(
                            seq(ylim[1], ylim[2], length.out = ncolor + 1),
                            "native"
                        )[-(ncolor + 1)],
                        x = unit(0, "npc") - unit(1, "points"),
                        width = 1,
                        height = 1 / ncolor,
                        gp = gpar(fill = palette, lty = 0),
                        just = c("left", "bottom")
                    )
                } else {
                    grid.rect(
