...
setMethod("initialize", "AnnotationTrack", function(.Object, ...) {
    ...
    if (!is.null(range) && length(.Object)) {
        ...
        grp <- if (is(.Object, "GeneRegionTrack")) {
            values(range)$transcript
        } else {
            values(range)$group
        }
        if (
            any(
                vapply(
                    split(as.character(strand(range)), grp),
            ...
            stop(
                "Grouped elments of a RangeTrack can not be on opposing strands"
            )
        }
    }
...
#' `AnnotationTrack` constructor to make the right choice.
#'
#' @name ReferenceAnnotationTrack-class
#' @exportClass ReferenceAnnotationTrack
#' @keywords internal
setClass(
    "ReferenceAnnotationTrack",
    contains = c("AnnotationTrack", "ReferenceTrack")
)
...
##      to a GRanges object, i.e., it needs at least the mandatory 'start',
##      'stop' and 'strand' columns. Additional optional columns are:
##        - feature: the type of the item. Can be mapped to colors via the
##                   DisplayPars.
##        - group: a grouping factor to connect track items.
##        - id: a unique identifier for a feature.
##              This will be plotted if showId==TRUE
##              Note that internally we use the value of ID as the seqnames
##              slot in the internal GRanges object. Defaults for all missing
##              columns are generated.
##    Instead of using the 'range' parameter, all these values can also be
##    passed as individual vectors, in which case they need to be of similar
## .   length.
...
##    o feature, group, id: individual vectors of equal length as described
## .                         above.
##    o genome, chromosome: the reference genome and active chromosome
##    o stacking: character controlling the stacking of overlapping items.
...
AnnotationTrack <- function(
  ...
  ...
    ...
    ## Pipes have a special meaning for merged groups,
    ## so we can't have them in the initial group vector
    mcols(range)[["group"]] <- gsub(
        "|",
        "",
        mcols(range)[["group"]],
        fixed = TRUE
    )
    ## If no chromosome was explicitly asked for
    ## we just take the first one in the GRanges object
    ...
    ## we have to distinguish between DetailsAnnotationTracks and normal ones
    ...
    if (missing(fun)) {
        if (!isStream) {
            ...
            ## A bit hackish, but for some functions we may want to know
            ## which track type we need but at the same time
            ## we do not want to enforce this as an additional argument
            e <- new.env()
...
#' @template DetailsAnnotationTrack_param
#'
#' @name DetailsAnnotationTrack-class
...
#'
#' @return The return value of the constructor function is a new object of
#' class [`DetailsAnnotationTrack`][DetailsAnnotationTrack-class].
#'
#' @examples
...
#' @describeIn DetailsAnnotationTrack-class Initialize the `fun` and `selectFun`
...
setReplaceMethod(
    "group",
    signature("AnnotationTrack", "character"),
    function(object, value) .setAnn(object, value, "group")
)

## Context-dependent meta-accessors to the identifier data of an AnnotationTrack
## and a GeneRegionTrack. For the former, those will be the content of the
## 'id', the 'group' or the 'feature' metadata column, for the latter, one in
## the selection of 'symbol', 'gene', 'transcript', 'feature' or 'exon'.
## For historical reasons, logical values are also allowed, where TRUE
## is equivalent to 'symbol' and FALSE is equivalent to 'gene'. If not provided
## explicitly in the 'type' argument, the identifier type is inferred from
## the 'transcriptAnnotation' display parameter (a.k.a. 'geneSymbols')
## for a GeneRegionTrack, and from the 'groupAnnotation' parameter for
## an AnnotationTrack. The special value 'lowest' should work across all
## classes and provided the most detailed annotation
## ('id' for AnnotationTrack and 'exon' for GeneRegionTrack).
...
setMethod(
    "setStacks",
    "AnnotationTrack",
    function(GdObject, recomputeRanges = TRUE) {
...
#' @describeIn AnnotationTrack-class Determine whether there is group label
#' annotation or not, and add this information as the internal display
#' parameter `.__hasAnno`. Precompute the grouped ranges together with
#' optional labels in order to determine the correct plotting range later.
#' @keywords internal
setMethod(
    "consolidateTrack",
    ...
    function(GdObject,
        ...
        if (length(GdObject)) {
            ## ids are shown if either set by the showId parameter,
            ## or through the use of the transcriptAnnotation
            ## or groupAnnotation parameters
            ids <- identifier(GdObject)
            sid <- .dpOrDefault(GdObject, "showId")
            ta <- if (is(GdObject, "GeneRegionTrack")) {
                .dpOrDefault(GdObject, "transcriptAnnotation")
            } else {
                .dpOrDefault(GdObject, "groupAnnotation")
            }
            sid <- if (is.null(sid)) !is.null(ta) && ta != "none" else sid
            hasAnno <- sid && !all(ids == "")
            ## element ids are shown if either the showFeatureId
            ## or showExonId parameters are TRUE, or if featureAnnotation
            ## or exonAnnotation is set
            sfid <- if (is(GdObject, "GeneRegionTrack")) {
                .dpOrDefault(GdObject, "showExonId")
            } else {
                .dpOrDefault(GdObject, "showFeatureId")
            }
            fa <- if (is(GdObject, "GeneRegionTrack")) {
                .dpOrDefault(GdObject, "exonAnnotation")
            } else {
                .dpOrDefault(GdObject, "featureAnnotation")
            }
...
## Find all elements in a GRanges object 'grange' with distance smaller than
## 'minXDist' and merge them along with their additional metadata columns.
## 'elements' is a frequency table of items per group, and it is needed to
## figure out whether all items of a given group have been merged.
## 'GdObject' is the input track object from which certain information has to
## be extracted. The output of this function is a list with elements
##   o range: the updated GRanges object
##   o needsRestacking: logical flag indicating whether stacks have to be
##                      recomputed
##   o split: the original merged annotation
##            (need to work on this, currently not used)
##   o merged: logical vector indicating which of the elements
## .            in 'range' constitute fully merged groups
.collapseAnnotation <- function(
  grange,
  minXDist,
  elements,
  GdObject,
  offset = 0
    ...
    if (length(rRed) < length(grange)) {
        ## Some of the items have to be merged and we need to make sure
        ## that the additional annotation data that comes with it
        ...
        ## Here we hijack the seqnames column to indicate
        ## whether the whole group has been merged
        if (nrow(newVals)) {
            newVals$seqnames <- elements[as.character(anno[
                identical,
                "seqnames"
            ])] ==
                1
            newVals$gdensity <- ifelse(
                elements[as.character(anno[identical, "seqnames"])] == 1,
                1,
                NA
            )
        }
        ...
        tmpVals <- lapply(seq_along(annoSplit), function(i) {
            ...
            if (is(GdObject, "GeneRegionTrack")) {
                c(
                    strand = ifelse(
                        length(unique(x[, "strand"])) == 1,
                        as.character(x[1, "strand"]),
                        "*"
                    ),
                    density = sum(as.integer(x[, "density"])),
                    gdensity = ifelse(
                        is.na(head(x[, "gdensity"], 1)),
                        1,
                        sum(as.integer(x[, "gdensity"]))
                    ),
                    feature = ifelse(
                        length(unique(x[, "feature"])) == 1,
                        as.character(x[1, "feature"]),
                        "composite"
                    ),
                    id = ifelse(
                        length(unique(x[, "id"])) == 1,
                        as.character(x[1, "id"]),
                        cid(i)
                    ),
                    start = min(x[, "start"]),
                    ...
                    gene = ifelse(
                        length(unique(x[, "gene"])) == 1,
                        as.character(x[1, "gene"]),
                        cid(i)
                    ),
                    exon = ifelse(
                        length(unique(x[, "exon"])) == 1,
                        as.character(x[1, "exon"]),
                        cid(i)
                    ),
                    transcript = ifelse(
                        length(unique(x[, "transcript"])) == 1,
                        as.character(x[1, "transcript"]),
                        cid(i)
                    ),
                    symbol = ifelse(
                        length(unique(x[, "symbol"])) == 1,
                        as.character(x[1, "symbol"]),
                        cid(i)
                    ),
                    rank = min(as.integer(x[, "rank"])),
                ...
                c(
                    ...
                    group = ifelse(
                        length(unique(x[, "group"])) == 1,
                        as.character(x[1, "group"]),
                        cid(i)
                    ),
                    seqnames = as.vector(nrow(x) == elements[x[1, "seqnames"]])
                )
        ...
        newVals <- rbind(
            newVals,
            as.data.frame(do.call(rbind, tmpVals), stringsAsFactors = FALSE)
        )
        merged <- as.logical(newVals$seqnames)
        grange <- GRanges(
            seqnames = chromosome(GdObject),
            strand = newVals[, "strand"],
            ranges = IRanges(
                start = as.integer(newVals[, "start"]),
                end = as.integer(newVals[, "end"])
            )
        )
        cnMatch <- match(
            c(colnames(values(GdObject)), "gdensity"),
            colnames(newVals)
        )
        mcols(grange) <-
            if (any(is.na(cnMatch))) {
                newVals[, setdiff(
                    colnames(newVals),
                    c("strand", "start", "end", "seqnames")
                )]
            } else {
                newVals[, cnMatch]
            }
    ...
    return(list(
        range = grange,
        needsRestacking = needsRestacking,
        split = annoSplit,
        merged = merged,
        offset = length(annoSplit)
    ))
}

## For AnnotationTracks we need to collapse the all regions along with
## the additional annotation. For GeneRegionTracks we essentially need to do
## the same thing as for AnnotationTracks, however the additional annotation
## columns are quite different. We do this in multiple turn with increasing
## levels of complexity:
##    1.) merge all individual items within a group that can no longer
##        be separated
##    2.) merge overlapping groups with just a single remaining item
##        (optional, if mergeGroups==TRUE)
...
setMethod(
    ...
    function(GdObject, diff = .pxResolution(coord = "x"), xrange) {
        ## We first add the original unmodified GdObject as a display parameter
        ## to be able to reference back if we ever need to
        displayPars(GdObject) <- list(
            ".__OriginalGdObject" = .deepCopyPars(GdObject)
        )
        ...
        r <- rNew
        ## Collapse all items within a group to a single meta-item
        ## (if collapseTranscripts is set)
        if (is(GdObject, "GeneRegionTrack")) {
            ...
            switch(ctrans,
                "gene" = {
                    newVals <- unlist(endoapply(
                        split(
                            values(r),
                            paste(gene(GdObject), strand(GdObject))
                        ),
                        head,
                        1
                    ))
                ...
                "longest" = {
                    r <- unlist(endoapply(
                        split(r, gene(GdObject)),
                        function(x) {
                            xs <- split(x, x$transcript)
                            xs[[which.max(vapply(
                                xs,
                                function(y) {
                                    abs(diff(as.numeric(as.data.frame(ranges(range(
                                        y
                                    )))[, c("start", "end")])))
                                },
                ...
                "meta" = {
                    ...
                    newVals <- newVals[
                        rep(seq_along(rtmp), elementNROWS(rtmp)),
                    ]
                    newVals$exon <- paste(
                        "merged_exon_",
                        unlist(
                            lapply(elementNROWS(rtmp), function(x) seq(1, x)),
                            use.names = FALSE
                        ),
                        sep = ""
                    )
        ...
        ## Collapse overlapping ranges (less than minXDist space between them)
        ## and process the annotation data
        if (collapse) {
            ## Merge all items in those groups for which no individual items
            ## can be separated
            elements <- table(group(GdObject))
            rr <- GRanges(
                seqnames = as.character(group(GdObject)),
                ranges = IRanges(start = start(r), end = end(r)),
                strand = strand(r)
            )
            mcols(rr) <- mcols(r)
            rr <- sort(unique(rr))
            ...
            ## Now we take a look whether there are any groups that could be
            ## merged (if mergeGroups is TRUE)
            if (
                .dpOrDefault(GdObject, "mergeGroups", FALSE) &&
                    any(mergedAnn$merged)
            ) {
                rr <- sort(mergedAnn$range[mergedAnn$merged])
                strand(rr) <- "*"
                mergedAnn2 <- .collapseAnnotation(
                    rr,
                    minXDist,
                    elements,
                    GdObject,
                ...
                mergedAnn$range <- c(
                    mergedAnn$range[!mergedAnn$merged],
                    mergedAnn2$range
                )
            }
            r <- mergedAnn$range
...
## In order to keep the grouping information for track regions in the clipped
## areas we have to keep all group elements that overlap with the range.
## We still want to record the requested ranges in the internal
## '.__plottingRange' display parameter.

#' @describeIn AnnotationTrack-class subset an `AnnotationTrack` by coordinates
...
setMethod(
    ...
    function(x,
        ...
        if (length(x)) {
            ...
            ## Now remove everything except for the overlapping groups
            ## by first subselecting all groups in the range...
            gsel <- names(granges)[subjectHits(findOverlaps(
                GRanges(
                    seqnames = chromosome(x),
                    ranges = IRanges(min(ranges), max(ranges))
                ),
                granges
            ))]
            x <- x[group(x) %in% gsel]
...
## ReferenceDataTracks need to stream the data from file
## and then pass the results on to the next method
...
setMethod(
    "subset",
    signature(x = "ReferenceAnnotationTrack"),
    function(x, from, to, chromosome, ...) {
        ## We only need to reach out into the referenced file once
        ## if the range is already contained in the object
        if (missing(from) || is.null(from) || missing(to) || is.null(to)) {
            stop(
                "Need both start and end location to subset a ReferenceAnnotationTrack"
            )
        }
        if (missing(chromosome) || is.null(chromosome)) {
...
#' @describeIn AnnotationTrack-class superpose a vertical grid across the
#' track's plotting area, based on the `grid`, `v`, `col.grid`, `lty.grid`
#' and `lwd.grid` display parameters.
setMethod(
    "drawGrid",
    signature(GdObject = "AnnotationTrack"),
    function(GdObject, from, to) {
...
##
## Calculate all coordinates and values for the individual stacks first,
## and append to the input list 'toDraw'. This allows us to plot the whole
## track at once, making use of grid's vectorization. Without this tweak,
## every stack would have to be drawn individually, which can be painfully
## slow...
## Because all of the y coordinates are calculated in the coordinate
## system of the current viewport for the respective stack we have
## to add offset values.

## Compute the coordinates and colors for all track items
## (e.g. exons in a GeneRegionTrack)
.boxes <- function(GdObject, offsets) {
    ...
    id[sel] <- sprintf(
        "%i merged\n%s",
        as.integer(.getAnn(GdObject, "density")[sel]),
        ifelse(
            class(GdObject) %in% c("AnnotationTrack", "DetailsAnnotationTrack"),
            "features",
            "exons"
        )
    )
    ...
    boxes <- data.frame(
        cx1 = start(GdObject),
        cy1 = yy1,
        cx2 = start(GdObject) + width(GdObject),
        cy2 = yy2,
        fill = color,
        strand = strand(GdObject),
        text = id,
        textX = start(GdObject) + (width(GdObject) / 2),
        textY = middle + offsets,
        .getImageMap(cbind(start(GdObject), yy1, end(GdObject), yy2)),
        start = start(GdObject),
        end = end(GdObject),
        values(GdObject),
        exonId = id,
        origExonId = identifier(GdObject, type = "lowest"),
...
## Compute the coordinates for the bars connecting grouped items
...
.barsAndLabels <- function(GdObject) {
    ...
    needBar <- vapply(grpSplit, length, FUN.VALUE = numeric(1L)) > 1 &
        width(grpRanges) > res
    ## If we draw the bar from start to end of the range we sometimes see
    ## little overlaps that extend beyond the first or last item.
    ## In order to fix this, we just subtract the equivalent
    ## of min.width pixels from both ends of each group range
    ...
    color <- if (length(grep("__desatCol", values(GdObject)$feature[1]))) {
        ...
        vapply(
            split(.getBiotypeColor(GdObject), gp),
            head,
            1,
            FUN.VALUE = character(1L)
        )
    ...
    bars <- data.frame(
        sx1 = start(grpRanges)[needBar],
        sx2 = end(grpRanges)[needBar],
        y = yloc[needBar],
        strand = strand[needBar],
        col = color[needBar],
        stringsAsFactors = FALSE
    )
    ...
    if (!is.null(labs)) {
        ...
        if (any(lsel)) {
            gdens <- as.integer(vapply(
                split(.getAnn(GdObject, "gdensity"), gp),
                head,
                1,
                FUN.VALUE = character(1L)
            ))
            labs[lsel] <- sprintf(
                ...
                ifelse(
                    class(GdObject) %in%
                        c("AnnotationTrack", "DetailsAnnotationTrack"),
                    "groups",
                    "transcript models"
                )
            )
        ...
        switch(just,
            "left" = {
                cx <- if (!rev) {
                    start(grpRanges) - sizes$after
                } else {
                    end(grpRanges) + sizes$after
                }
                cy <- yloc
                algn <- c("right", "center")
            },
...
setMethod(
    "drawGD",
    signature("AnnotationTrack"),
    function(GdObject, minBase, maxBase, prepare = FALSE, subset = TRUE, ...) {
        imageMap(GdObject) <- NULL
        if (!length(GdObject)) {
            return(invisible(GdObject))
        }
        ## In prepare mode we need to make sure that the stacking information
        ## is updated from the optional display parameter (by calling the
        ## StackedTrack drawGD method) and also perform the collapsing of
        ## track items which could potentially lead to re-stacking.
        if (prepare) {
            GdObject <- callNextMethod(GdObject, ...)
            bins <- stacks(GdObject)
            stacks <- max(bins)
            ## We need to collapse the track object based on the current
            ## screen resolution (note that this may trigger re-stacking)
            pushViewport(dataViewport(
                xData = c(minBase, maxBase),
                extension = 0,
                yscale = c(1, stacks + 1),
                clip = TRUE
            ))
        ...
        ## If there are too many stacks for the available device resolution
        ## we cast an error, otherwise we set the viewport
        bins <- stacks(GdObject)
        stacks <- max(bins)
        rev <- .dpOrDefault(GdObject, "reverseStrand", FALSE)
        xscale <- if (!rev) c(minBase, maxBase) else c(maxBase, minBase)
        yscale <- if (!.dpOrDefault(GdObject, "reverseStacking", FALSE)) {
            c(1, stacks + 1)
        } else {
            c(stacks + 1, 1)
        }
        ...
        curVp <- vpLocation()
        if (
            curVp$size["height"] / stacks <
                .dpOrDefault(GdObject, "min.height", 3)
        ) {
            stop(
        ...
        if (.dpOrDefault(GdObject, "showOverplotting", FALSE)) {
            ...
            if (length(unique(dens)) != 1) {
                ...
                saturation <- minSat +
                    ((dens - minDens) / rDens / (1 / (1 - minSat)))
                bc <- unique(.getBiotypeColor(GdObject))
                baseCol <- rgb2hsv(col2rgb(bc))
                desatCols <- unlist(lapply(saturation, function(x) {
                    hsv(baseCol[1, ], x, baseCol[3, ])
                }))
                names(desatCols) <- paste(
                    unique(feature(GdObject)),
                    rep(dens, each = length(bc)),
                    sep = "_"
                )
        ...
        ## Now we can pre-compute all the coordinates and settings
        ## for the elements to be drawn, ...
        ...
        if (nrow(box) > 0) {
            ## If we want to place a label on top or below the ranges
            ## we need to know how much size that will take up
            ## and adjust the plotting shapes accordingly
            drawLabel <- .dpOrDefault(GdObject, ".__hasAnno", FALSE) &&
                !is.null(bartext) &&
                !anyNA(bartext) &&
                nrow(bartext) > 0 &&
                stacking(GdObject) != "dense"
            ...
            if (drawLabel && just %in% c("above", "below")) {
                ...
                labelSpace <- as.numeric(convertHeight(
                    unit(2, "points"),
                    "native"
                ))
                ...
                if (just == "above") {
                    ...
                    bartext$y <- bartext$y + labelSpace
                    box$cy1 <- bs +
                        ((box$cy1 %% 1 - avSpace) * sfac) +
                        avSpace +
                        vadjust
                    box$cy2 <- bs +
                        ((box$cy2 %% 1 - avSpace) * sfac) +
                        avSpace +
                        vadjust
                }
            }
            ...
            if (nrow(bar) > 0) {
                .arrowBar(
                    ...
                    y = bar$y,
                    bar$strand,
                    box[, seq_len(4), drop = FALSE],
                    W = .dpOrDefault(GdObject, "arrowFeatherWidth", 3),
                    D = .dpOrDefault(GdObject, "arrowFeatherDistance", 20),
                    col = if (is.null(col.line)) {
                        bar$col
                    } else {
                        rep(col.line, length(bar$col))
                    },
                    lwd = lwd,
                    lty = lty,
                    alpha = alpha,
                    barOnly = (!"smallArrow" %in%
                        .dpOrDefault(GdObject, "shape", "box") ||
                        stacking(GdObject) == "dense"),
                    diff = res,
                    min.height = .dpOrDefault(GdObject, "min.height", 3)
                )
            }
            ...
            if (
                "box" %in%
                    ...
                    ("smallArrow" %in%
                        shape &&
                        !("arrow" %in% shape || "ellipse" %in% shape))
            ) {
            ...
            if ("ellipse" %in% shape) {
                ...
                grid.polygon(
                    ...
                    id = ellCoords$id,
                    gp = gpar(
                        col = if (is.na(border)) box$fill else border,
                        fill = box$fill,
                        lwd = lwd,
                        lty = lty,
                        alpha = alpha
            ...
            if (.dpOrDefault(GdObject, "showFeatureId", FALSE)) {
                ...
                grid.text(
                    ...
                    box$textY,
                    rot = rotation,
                    gp = .fontGp(GdObject, subtype = "item"),
                    default.units = "native",
            ...
            if (drawLabel) {
                grid.text(
                    ...
                    bartext$y,
                    rot = rotation.group,
                    gp = .fontGp(GdObject, subtype = "group"),
                    default.units = "native",
...
## Create a data.frame with the distinct details function arguments
## (like start, end, ...)
.buildArgsDf <- function(GdObject) {
    groupDetails <- .dpOrDefault(GdObject, "groupDetails", FALSE)
    rr <- if (groupDetails) {
        unlist(range(split(ranges(GdObject), group(GdObject))))
    } else {
        ranges(GdObject)
    }
    args <- data.frame(
        start = as.integer(start(rr)),
        end = as.integer(end(rr)),
        ...
        identifier = as.character(
            if (groupDetails) {
                ...
                identifier(GdObject, type = "lowest")
            }
        ),
        stringsAsFactors = FALSE
    )
...
setMethod(
    "drawGD",
    signature("DetailsAnnotationTrack"),
    function(GdObject, minBase, maxBase, prepare = FALSE, ...) {
        ...
        if (prepare) {
            ...
            indices <- if (groupDetails) {
                seq_len(length(unique(group(GdObject))))
            } else {
                seq_len(length(GdObject))
            }
            ...
            select <- lapply(indices, function(i) {
                ...
                iargs$GdObject.original <- .dpOrDefault(
                    GdObject,
                    ".__OriginalGdObject",
                    GdObject
                )
            ...
            if (any(select[, "warn"])) {
                warning(
                    "The result of function 'selectFun' has to be a single logical value. Forcing the value to 'TRUE'"
                )
            }
            select <- select[, "res"]
            popViewport(1)
        ...
        cex <- rep(
            .dpOrDefault(
                GdObject,
                "detailsConnector.cex",
                fromPrototype = TRUE
            ),
            n
        )[seq_len(n)]
        border.lty <- rep(
            .dpOrDefault(GdObject, "detailsBorder.lty", fromPrototype = TRUE),
            n
        )[seq_len(n)]
        border.lwd <- rep(
            .dpOrDefault(GdObject, "detailsBorder.lwd", fromPrototype = TRUE),
            n
        )[seq_len(n)]
        border.col <- rep(
            .dpOrDefault(GdObject, "detailsBorder.col", fromPrototype = TRUE),
            n
        )[seq_len(n)]
        border.fill <- rep(
            .dpOrDefault(GdObject, "detailsBorder.fill", fromPrototype = TRUE),
            n
        )[seq_len(n)]
        minwidth <- .dpOrDefault(
            GdObject,
            "details.minWidth",
            fromPrototype = TRUE
        )
        ...
        selection <- .dpOrDefault(
            GdObject,
            ".__select",
            rep(TRUE, length(GdObject))
        )
        len <- sum(selection)
        bins <- if (!groupDetails) {
            stacks(GdObject)
        } else {
            unlist(lapply(split(stacks(GdObject), group(GdObject)), unique))
        }
        stacks <- max(bins)
        if (len > 0) {
            if (
                ((maxBase - minBase) / len) / .pxResolution(coord = "x") <
                    minwidth
            ) {
                warning(
                    "too much detail for available space (plot fewer annotation or increase details.minWidth)!"
                )
                popViewport(1)
                GdObject <- callNextMethod()
                return(GdObject)
            ...
            for (i in indices[selection]) {
                ...
                args <- c(args[setdiff(names(args), names(iargs))], iargs)
                pres[[as.character(j)]] <- try(
                    do.call(GdObject@fun, args),
                    silent = TRUE
                )
                if (
                    !is.null(pres) && is(pres[[as.character(j)]], "try-error")
                ) {
                    hasError <- TRUE
                    grid.segments(
                        x0 = c(0.1, 0.1),
            ...
            if (hasError) {
                warning(
                    "Problems occurred in the detail plotting function:\n",
                    paste(pres, collapse = "\n")
                )
            }
            popViewport(1)
            ...
            GdObject <- callNextMethod()
            grid.segments(
                x0 = unit(xloc1[selection], "native"),
                x1 = xloc2,
                y0 = unit(yloc1[selection], "native"),
                y1 = yloc2,
                gp = gpar(col = col, lwd = lwd, lty = lty, cex = cex)
            )
            grid.points(
                x = unit(xloc2, "npc"),
                y = unit(yloc2, "npc"),
            ...
            grid.points(
                x = unit(xloc1[selection], "native"),
                y = unit(yloc1[selection], "native"),
                gp = gpar(col = col, cex = cex),
                pch = pch
            )
            popViewport(1)
    ...
    function(from, to) {
        ...
        new(
            ...
            rtracklayer::GenomicData(
                ...
                id = gsub(" ", "_", vals$id),
                name = gsub(" ", "_", as.character(vals$id)),
                itemRgb = color,
                strand = strand
            ),
            trackLine = line
...
setAs("GRanges", "AnnotationTrack", function(from, to) {
    AnnotationTrack(range = from)
})

setAs("GRangesList", "AnnotationTrack", function(from, to) {
    AnnotationTrack(range = from)
})
...
## We have to show the name, genome and currently active chromosome,
## and, if more ranges are available on additional chromosomes some
...
setMethod(
    "show",
    signature(object = "ReferenceAnnotationTrack"),
    function(object) {
