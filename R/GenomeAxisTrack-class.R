...
#' items.
#' @export
setMethod("start", "GenomeAxisTrack", function(x) {
    if (length(x)) start(range(x)) else NULL
})
...
#' items.
#' @export
setMethod("end", "GenomeAxisTrack", function(x) {
    if (length(x)) end(range(x)) else NULL
})
...
#' genomic coordinates.
#' @export
setMethod("width", "GenomeAxisTrack", function(x) {
...
#' @export
setMethod("values", "GenomeAxisTrack", function(x) {
    as.data.frame(values(ranges(x)))
})
...
#' @export
setMethod("strand", "GenomeAxisTrack", function(x) {
    as.character(strand(ranges(x)))
})
...
setMethod(
    "subset",
    signature(x = "GenomeAxisTrack"),
    function(x, from = NULL, to = NULL, sort = FALSE, ...) {
        if (!length(x)) {
            return(x)
        }
...
setMethod(
    ...
    function(GdObject, minBase, maxBase, prepare = FALSE, subset = TRUE, ...) {
        ...
        endMargin <- if (add53 || add35) {
            abs(as.numeric(convertWidth(stringWidth("5'"), "native")) * lcex) +
                (textXOff * 2)
        } else {
            pres["x"] * 5
        }
        ...
        lwdAdd <- (lwd - 1) / 2
        tickHeight <- (ifelse(littleTicks, 2, 1) * 3 * dfact + lwdAdd) *
            pres["y"]
        ids <- as.character(values(GdObject)$id)
        showIds <- .dpOrDefault(GdObject, "showId", FALSE) &&
            !is.null(ids) &&
            !all(ids == "")
        ...
        sep <- (if (length(GdObject)) {
            if (showIds) {
                max(
                    1.5,
                    ((max(
                        as.numeric(convertHeight(stringHeight(ids), "native")) *
                            rcex
                    ) +
                        textYOff) /
                        pres["y"]) /
        ...
        if (!is.null(scaleLen)) {
            ...
            if (scaleLen > len) {
                warning(sprintf(
                    "scale (%d) cannot be larger than plotted region %d - setting to ~5%\n",
                    scaleLen,
                    len
                ))
                scaleLen <- 0.05
            ...
            labelPos <- match.arg(
                labelPos,
                c("alternating", "revAlternating", "above", "below", "beside")
            )
            if (scaleLen <= 1 && scaleLen > 0) {
                # calculate and round the scale
                scaleLen <- len * scaleLen
                ex <- .dpOrDefault(GdObject, "exponent", floor(log10(scaleLen)))
                v <- round(scaleLen, -ex)
                if (v == 0) v <- scaleLen
            } else {
                # if the scale is an absolute value don't round
                ex <- .dpOrDefault(GdObject, "exponent", floor(log10(scaleLen)))
                v <- scaleLen
            }
            ...
            grid.lines(
                ...
                gp = gpar(col = color, lwd = lwd, alpha = alpha)
            )
            grid.segments(
                x0 = c(xoff, v + xoff),
                y0 = c(0 - tickHeight, 0 - tickHeight),
                x1 = c(xoff, v + xoff),
                y1 = c(tickHeight, tickHeight, tickHeight),
                default.units = "native",
                gp = gpar(col = color, lwd = lwd, alpha = alpha)
            )
            z <- len * 0.01
            if (labelPos == "below") {
                grid.text(
                    label = if (is.character(label)) label else label[[1]],
                    x = xoff + v / 2,
                    y = 0 - (tickHeight / 1.5 * dfact),
                    just = c("center", "top"),
                    gp = gpar(
                        alpha = alpha,
                        col = color,
        ...
        GdObject <- GdObject[
            end(GdObject) > axRange[1] & start(GdObject) < axRange[2]
        ]
        if (length(GdObject)) {
            ...
            grid.rect(
                ...
                default.units = "native",
                just = c("left", "bottom"),
                gp = gpar(col = rcolor, fill = rfill, alpha = alpha)
            )
            vals <- values(GdObject)
            if (showIds) {
                grid.text(
                    ids,
                    x = start(GdObject) + width(GdObject) / 2,
                    y = 0,
            ...
            tags <- lapply(
                list(
                    title = ids,
                    start = as.character(start(GdObject)),
                    end = as.character(end(GdObject))
                ),
                function(x) {
        ...
        grid.segments(
            ...
            y1 = c(-1, 1) * pyOff,
            default.units = "native",
            gp = gpar(col = color, alpha = alpha, lwd = lwd)
        )
        grid.segments(
            x0 = c(axRange[2] - pxOff, axRange[1]),
            y0 = c(pyHook, -pyOff),
            x1 = c(axRange[2], axRange[1] + pxOff),
            y1 = c(pyOff, -pyHook),
            default.units = "native",
            gp = gpar(col = color, alpha = alpha, lwd = lwd)
        )
        ...
        y1t <- y0t +
            rep(c(tickHeight, -tickHeight), length(tck))[seq_along(tck)]
        labelPos <- match.arg(
            labelPos,
            c("alternating", "revAlternating", "above", "below", "beside")
        )
        ...
        ylabs <- y1t +
            (ifelse(y1t > 0, 1, -1) *
                (textYOff +
                    (as.numeric(convertHeight(stringHeight("1"), "native")) /
                        2) *
                        cex))
        if (is.character(label)) {
            grid.text(
                ...
                gp = gpar(cex = cex, fontface = fontface),
                default.units = "native"
            )
        } else {
        ...
        if (.dpOrDefault(GdObject, "littleTicks", FALSE) && length(tck) > 1) {
            ...
            spaceNeeded <- min(
                as.numeric(convertWidth(
                    stringWidth(
                        if (is.character(label)) label else "000000000"
                    ),
                    "native"
                )) /
                    2
            ...
            for (i in seq_len(length(btck) - 1)) {
                toFill <- btck[i:(i + 1)]
                ttck <- if (i == 1) {
                    rev(toFill[2] - (avSpace / nTcks) * seq_len(nTcks - 1))
                } else {
                    toFill[1] + (avSpace / nTcks) * seq_len(nTcks - 1)
                }
                ltck <- c(ltck, ttck)
                ord <- if (i == 1) {
                ...
                y1 <- y0 +
                    rep(ord * tickHeight / 2, length(ttck))[seq_along(ttck)]
                y0lt <- c(
                    y0lt,
                    switch(labelPos,
                        "alternating" = y0,
            ...
            sel <- ltck > min(tck, axRange + endPadding) &
                ltck < max(tck, axRange - endPadding)
            if (length(ltck[sel]) && min(diff(tck)) > nTcks) {
                ...
                if (is.character(label)) {
                    grid.text(
                        ...
                        y = ytlabs[sel],
                        just = c("centre", "centre"),
                        gp = gpar(cex = lcex, fontface = fontface),
                        default.units = "native"
                    )
                } else {
        ...
        if (add53) {
            grid.text(
                label = p5,
                x = min(axRange) - textXOff,
                y = pyOff,
                just = c(ifelse(rev, "left", "right"), "bottom"),
                gp = gpar(cex = cex * .75, fontface = fontface),
                default.units = "native"
            )
            grid.text(
                label = p3,
                x = max(axRange) + textXOff,
                y = pyOff,
                just = c(ifelse(rev, "right", "left"), "bottom"),
                gp = gpar(cex = cex * .75, fontface = fontface),
                default.units = "native"
            )
        }
        if (add35) {
            grid.text(
                label = p3,
                x = axRange[1] - textXOff,
                y = -pyOff,
                just = c(ifelse(rev, "left", "right"), "top"),
                gp = gpar(cex = cex * .75, fontface = fontface),
                default.units = "native"
            )
            grid.text(
                label = p5,
                x = axRange[2] + textXOff,
                y = -pyOff,
                just = c(ifelse(rev, "right", "left"), "top"),
                gp = gpar(cex = cex * 0.75, fontface = fontface),
                default.units = "native"
            )
        }
        popViewport()
