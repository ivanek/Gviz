                            ...
                            id, strand="*", chromosome, genome, stacking="squish", name="AnnotationTrack", ...)
{
    ...
    return(new("AnnotationTrack", chromosome=chromosome, range=range,
...
GeneRegionTrack <- function(range=NULL, rstarts=NULL, rends=NULL, rwidths=NULL, strand="*",
                            feature="unknown", exon=feature, transcript=feature, gene=feature, symbol=feature, chromosome,
                            genome, stacking="squish", name="GeneRegionTrack", start, end, ...)
{
    ## Some default checking
    if(missing(chromosome))
        stop("A chromosome must be specified for this annotation track.")
    ...
    chromosome <- .chrName(chromosome)[1]
    ## Build a GRanges object from the inputs
    range <- .buildRange(range=range, start=rstarts, end=rends, width=rwidths,
...
##Êcompared to features of an AnnotationTrack it only makse sense to prove details
##Êfor an AnnotationTrack with few features. The details plots are distributed evenly
##Êon top of the annotation and can be connected to their corresponding featurs (they're
##Êproviding details for) with lines for better readability.
##
...
##
## foo = function(...) {
##Ê   plot(densityplot(rnorm(1000), xlab=NA, ylab=NA), newpage=FALSE, prefix="foo")
## }
##
## Note, use plot with newpage=FALSE and an explicit prefix (otherwise plotting
##Êget more and more slow the more trellis plots you create!). See '?plot.trellis'
##Êfor details.
##----------------------------------------------------------------------------------------------------------------------
## (N)
setClass("DetailsAnnotationTrack",
		contains="AnnotationTrack",
		representation=representation(fun="function"),
...
DetailsAnnotationTrack <- function(range=NULL, start=NULL, end=NULL, width=NULL, feature, group,
		id, strand="*", chromosome, genome, stacking="squish", name="DetailsAnnotationTrack",
		fun=function(...){}, ...)
{
	## Some default checking
	if(missing(chromosome))
	...
	range <- .buildRange(range=range, start=start, end=end, width=width,
			...
			chromosome=chromosome)
	## And finally the object instantiation
	return(new("DetailsAnnotationTrack", chromosome=chromosome, range=range,
					name=name, genome=genome, stacking=stacking, fun=fun, ...))
}

setMethod("initialize", "DetailsAnnotationTrack", function(.Object, fun, ...) {
			## the diplay parameter defaults
