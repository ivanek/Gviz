...
test_that("values accessors and replacement methods work", {
    ...
    expect_identical(
        values(axisTrack),
        data.frame(score = 1, stringsAsFactors = FALSE)
    )
    ...
    expect_error(
        values(dataTrack) <- matrix("3", dimnames = list("score", NULL)),
        "Not numeric or dimensions of replacement value do not match."
    )
    expect_error(
        values(dataTrack) <- c(3, 3),
        "Not numeric or invalid length of replacement vector."
    )
})

test_that("subseq works", {
    expect_error(
        subseq(SequenceTrack(), start = NA, end = NA, width = 3),
        "Two out of the three in"
    )
    expect_error(
        subseq(SequenceTrack(), start = 10, end = 1),
        "'end' has to be bigger than 'start'"
    )
    expect_error(
        subseq(seqTrack.dna, start = 10, end = 1),
        "'end' has to be bigger than 'start'"
    )
    expect_error(
        subseq(SequenceTrack(), start = 1, end = 10e6 + 2),
        "Sequence is too big"
    )
    expect_error(
        subseq(seqTrack.bs, start = 1, end = 10e6 + 2),
        "Sequence is too big"
    )
    expect_warning(
        subseq(seqTrack.dna, start = 1, end = 10, width = 10),
        "are provided, ignoring"
    )
    expect_identical(
        as.character(subseq(SequenceTrack(), start = 1, end = 10)),
        as.character(DNAString("----------"))
    )
    expect_identical(
        as.character(subseq(seqTrack.dna, start = 1, end = 10)),
        as.character(DNAString("ATTTCCCTGA"))
    )
    expect_identical(
        as.character(subseq(seqTrack.dna, start = 1, width = 10)),
        as.character(DNAString("ATTTCCCTGA"))
    )
    expect_identical(
        as.character(subseq(seqTrack.dna, end = 10, width = 10)),
        as.character(DNAString("ATTTCCCTGA"))
    )
    expect_identical(
        as.character(subseq(seqTrack.dna, start = 91, end = 110)),
        as.character(DNAString("ACGTCTTCCA----------"))
    )
    expect_identical(
        as.character(subseq(seqTrack.bs, start = 1, width = 10)),
        as.character(DNAString("NNNNNNNNNN"))
    )
    expect_identical(
        as.character(subseq(seqTrack.dna, start = 1)),
        as.character(dna.sq[[1]])
    )
    expect_identical(
        as.character(subseq(SequenceTrack(), start = 1)),
        as.character(DNAString("-"))
    )
    expect_identical(
        as.character(subseq(seqTrack.dna, end = 1)),
        as.character(DNAString("A"))
    )
    expect_identical(
        as.character(subseq(SequenceTrack(), end = 1)),
        as.character(DNAString("-"))
    )
    displayPars(seqTrack.dna)$complement <- TRUE
    expect_identical(
        as.character(subseq(seqTrack.dna, start = 1, end = 10)),
        as.character(DNAString("TAAAGGGACT"))
    )

    expect_error(
        subseq(SequenceTrack(fastafile, chromosome = "chr1")),
        "Two out of the three in"
    )
    expect_error(
        subseq(SequenceTrack(fastafile, chromosome = "chr1"), start = 1),
        "Two out of the three in"
    )
    expect_warning(
        as.character(subseq(
            SequenceTrack(fastafile, chromosome = "chr1"),
            start = 1,
            end = 10,
            width = 10
        )),
    ...
    expect_identical(
        ...
        as.character(DNAString("CTANGAGACG"))
    )
    expect_identical(
        as.character(subseq(
            SequenceTrack(fastafile, chromosome = "chr1"),
...
test_that("strand accessors and replacement methods work", {
    ...
    expect_identical(strand(annoTrack), rep("+", 2))
    expect_error(
        strand(annoTrack) <- rep("+", 3),
        "Invalid replacement value or length of replacement"
    )
})
...
test_that("subsetting with [ works", {
    ...
    expect_identical(
        axisTrack[1]@range,
        GRanges("chr1", IRanges(1, 5), score = 1)
    )
    ...
    expect_identical(
        dataTrack[, 1, ]@data,
        matrix(1, nrow = 1, dimnames = list("score", NULL))
    )
    expect_error(
        dataTrack[, 2, ]@data,
...
test_that("splitting works", {
    ...
    expect_identical(
        split(dTrack, f = c("a", "b"))[[1]]@data,
        dTrack@data[, 1, drop = FALSE]
    )
    expect_identical(split(dTrack, f = c("a", "b"))[[2]]@range, dTrack@range[2])
    expect_identical(
        split(dTrack, f = c("a", "b"))[[2]]@data,
        dTrack@data[, 2, drop = FALSE]
    )
})
...
test_that("gene, symbol, transcript, feature and exon accessors and replacement methods work", {
    ...
    expect_identical(
        gene(geneTrack),
        paste0(as.character(geneModels$gene), ".1")
    )
    ...
    expect_identical(
        symbol(geneTrack),
        paste0(as.character(geneModels$symbol), ".1")
    )
    ...
    expect_identical(
        ...
        paste0(as.character(geneModels$transcript), ".1")
    )
    ...
    expect_identical(
        exon(geneTrack),
        paste0(as.character(geneModels$exon), ".1")
    )
    ...
    expect_identical(
        feature(geneTrack),
        paste0(as.character(geneModels$feature), ".1")
    )
...
test_that("group and identifier accessors and replacement methods work", {
    ...
    expect_identical(
        identifier(geneTrack, type = NULL),
        as.character(geneModels$symbol)
    )
    identifier(geneTrack) <- paste0(as.character(geneModels$symbol), ".1")
    expect_identical(
        identifier(geneTrack),
        paste0(as.character(geneModels$symbol), ".1")
    )
...
test_that("stacking works", {
    ...
    expect_identical(
        Gviz:::setStacks(AnnotationTrack(c(gr)))@stacks,
        c(unknown = 1L)
    )
})
...
test_that("imageMap and coords work", {
    ...
    expect_identical(
        coords(im),
        matrix(1, ncol = 4, nrow = 1, dimnames = list("first"))
    )
    expect_identical(
        coords(annoTrack),
        matrix(1, ncol = 4, nrow = 1, dimnames = list("first"))
...
test_that("consolidating of tracks works", {
    ...
    expect_identical(
        getPar(
            consolidateTrack(annoTrack, chromosome = "chr2", alpha = TRUE),
            ".__hasAlphaSupport"
        ),
        TRUE
    )
...
test_that("collapsing of tracks works", {
    ...
    expect_identical(
        Gviz:::.myFindOverlaps(gr, GRanges("chr2", IRanges(1, 5))),
        integer()
    )
})
