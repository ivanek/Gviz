...
test_that("checking the class and structure works", {
    ...
    expect_error(
        .checkClass(data.frame(), "data.frame", length = 2),
        "of length"
    )
})
...
test_that("conversion of chromosome names works", {
    ...
    expect_identical(
        .chrName(c("1", "chr1", "M", "MT", "X")),
        c("chr1", "chr1", "chrM", "chrM", "chrX")
    )
...
test_that("estimating of coordinates for an HTML image map works", {
    ...
    mm <- matrix(
        rep(as.numeric(c(0, 1, 504, 503)), 2),
        ncol = 4,
...
test_that("deep copying of displaypars works", {
    ...
    expect_identical(
        displayPars(.deepCopyPars(annoTrack)),
        displayPars(annoTrack)
    )
})
...
test_that("import of sequence from FASTA file works", {
    expect_identical(
        .import.fasta(fastafile, GRanges("chr3", IRanges(1, 4))),
        DNAStringSet()
    )
    expect_identical(
        .import.fasta(fastafile, GRanges("chr1", IRanges(1, 4))),
        DNAStringSet(c("chr1" = "CTAN"))
    )
})
...
test_that("import of sequence from 2bit file works", {
    ...
    expect_identical(
        .import.2bit(twobitfile, GRanges("chr3", IRanges(1, 4))),
        DNAStringSet()
    )
    expect_identical(
        .import.2bit(twobitfile, GRanges("chr1", IRanges(1, 4))),
        DNAStringSet(c("chr1" = "CTAN"))
    )
})
...
test_that("import of alignments from BAM file works", {
    ...
    expect_error(
        .import.bam.alignments(
            bamfile,
            GRanges("chr1", IRanges(189891401, 189894000))
        ),
        "Unable to find index for BAM file"
    )
    bamfile <- Rsamtools::asBam(
        samfile,
    ...
    expect_identical(
        .import.bam.alignments(bamfile, GRanges("chr2", IRanges(1, 2))),
        empty
    )
    seqlevels(empty) <- "chr1"
    expect_identical(
        .import.bam.alignments(bamfile, GRanges("chr1", IRanges(1, 2))),
        empty
    )
})
...
test_that("conversion of ranges to summarizedJunctions works", {
    ...
    range <- GRanges(
        ...
        IRanges(1, 10),
        cigar = "1M8N1M",
        readStrand = Rle(factor("+", levels = c("+", "-", "*"))),
        entityId = 1
    )
    juns <- GRanges(
        "chr1",
        IRanges(start = 2, end = 9),
        score = 1L,
        plus_score = 1L,
        minus_score = 0L
    )
    expect_identical(
        .create.summarizedJunctions.for.sashimi.junctions(range),
        juns
    )
...
test_that("conversion of junction to list for plotting works", {
    ...
    expect_identical(
        .convert.summarizedJunctions.to.sashimi.junctions(juns),
        out
    )
    ...
    ## filter match
    expect_identical(
        .convert.summarizedJunctions.to.sashimi.junctions(juns, filter = filt),
        out
    )
    ## filter match (none)
    expect_identical(
    ...
    expect_identical(
        .convert.summarizedJunctions.to.sashimi.junctions(
            juns,
            filter = filt2,
            filterTolerance = 1
        ),
    ...
    expect_error(
        .convert.summarizedJunctions.to.sashimi.junctions(juns, trans = 1),
        "must be a function with a single argument"
    )
    expect_error(
        .convert.summarizedJunctions.to.sashimi.junctions(
            juns,
